...
#define CONFIG_VIDEO_DIAG_DISPLAY_TEST_FUNC 1
...
static void display_test_image_checkers(void)
{
    ...
    for (uint8_t* it = g_video_signal.frame_buffer; it < g_video_signal.frame_buffer + g_video_signal.frame_buffer_size_bytes; it += 10) 
    {
        memset(it, 0xFF, 20);
